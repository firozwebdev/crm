...
class Fields {
	...
	function hourmin($hid = "hour", $mid = "minute", $pid = "pm", $hval = "", $mval = "", $clock, $disabled=false,$js) 
	{ 
	    if(empty($hval)) $hval=8;
	    $time=explode('.',$clock);
	    $hval=$time[0]; $mval=$time[1];
	    #if(empty($mval)) $mval = date("i"); 
	    #if(empty($pval)) $pval = date("a"); 

	    if($js===true) $js = "onchange=\"javascript: figure_time();\"";
	    if(isset($disabled)) $disable="disabled=\"disabled\"";
	    $hours = array( '1'=>' 1 am', '2'=>' 2 am', '3'=>' 3 am','4'=>' 4 am', '5'=>' 5 am', '6'=>' 6 am', '7'=>' 7 am','8'=>' 8 am','9'=>' 9 am','10'=>'10 am','11'=>'11 am','12'=>'12 pm','13'=>' 1 pm','14'=>' 2 pm','15'=>' 3 pm','16'=>' 4 pm','17'=>' 5 pm','18'=>' 6 pm','19'=>' 7 pm','20'=>' 8 pm','21'=>' 9 pm','22'=>'10 pm','23'  =>'11 pm','24'=>"12 am"); 
	    $out = "<select name='$hid' id='$hid' $disable $js>"; 

	    ksort($hours);
	    foreach($hours as $v=>$hour){ 

	      if($hval == $v) $out .= "<option value='$v' SELECTED $disable >$hour</option>";
	      $out .= "<option value='$v' $sel $disable >$hour</option>";
	    }

	    $out .= "</select>&nbsp;&nbsp;"; 
	    $minutes = array(".00"=>0,".1"=>6,".2"=>12,".3"=>18,".4"=>24,".5"=>30,".6"=>36,".7"=>42,".8"=>48,".9"=>54); 
	    $out .= "<select name='$mid' id='$mid' $disable $js>";

	    foreach($minutes as $v => $minute) { 

	      if($mval == $v) $out .= "<option value='$v' SELECTED $disable >$minute</option>"; 
	      $out .= "<option value='$v' $sel $disable>$minute</option>";
	    }
 	...
 	function states($name, $selected='')
	{
	      $return = '<select name="'.$name.'" id="'.$name.'" class="input-xlarge">';
	              ...
	              foreach ($states as $abv => $full) {
	                  if($abv==$selected) $sel="selected"; else $sel='';
	                  $return .= '<option value="'.$abv.'" '.$sel.' >'.$full.'</option>';
	              }
	          $return .= '</select>';
	          return $return;
	}
    ...
    {           
    	...
    	if( !is_numeric($id) ) return $id;
    	   ...
    	   $query = $this->db->query( "SELECT value_label FROM new_claims_values WHERE client_id = ".$client_id." AND value_id = ".mysql_escape_string( trim( $id ) ) );
    	   return $query->result(); 
        }
    }
    ...
    {                          
    	$client_id = client_id();
    	echo "select * from  ".dbpre('values')." where  ".dbpre('values').".client_id = ".$client_id." AND list_id=$id";
    	$query = $this->db->query("select * from  ".dbpre('values')." where client_id = ".$client_id." AND list_id=$id" );
    	if( $query->result() ) return $query->result();  
    }
    function val_list_id($name) 
    {                          
    	$client_id = client_id();
    	$query=$this->db->query("select value_id from  ".dbpre('values_list')." where list_name='".mysql_escape_string( $name )."'");
        $row = $query->row();
		return $row->value_id;
    }
    ...
    { 
    	$client_id = client_id();
    	$query = $this->db->query("select list_name from  ".dbpre('values_list')." where client_id = ".$client_id." AND list_id=$id");
    	if( $query->result() ) return $query->result(); 
    	
    }
    function val_save($table,$values) 
    {        
    	$values->client_id=$this->session->userdata('client_id');
    ...
    function val_update($table,$values,$id)
    {
        ...
        if( $this->db->update($table,$values,$id) ) return $values->value_id;
    	else
    	{
    		echo 'DB ERROR';
    		exit();
